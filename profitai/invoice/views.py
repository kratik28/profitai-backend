...
class InvoiceOrderAPI(APIView):
    ...
    def post(self, request):
        try:
            ...
            remaining_total = request.data.get("remaining_total")
            sub_total = request.data.get("sub_total")
            status = None
            if payment_type == "pay_letter":
                status = 1
                remaining_total = remaining_total or grand_total
            elif payment_type == "remain_payment":
                status = 2
                remaining_total = remaining_total or (grand_total - paid_amount)
            elif payment_type == "paid":
                status = 3
                remaining_total = remaining_total or 0.00
            else:
                return Response({ "status_code": 200,
                                 "status": "error",
                                 "message": "please select valid payment type"})

            product_ids = []
            for i in product_quantity:
                prod = Product.objects.filter(id = i["productId"]).first()
                if prod.remaining_quantity<= 0 :
                      return Response({"status_code": 200,
                                    "status": "Error",
                                    "message":" Product out of stock. Please update your inventory."})

                elif prod.remaining_quantity> 0 and  i["quantity"]>prod.remaining_quantity:
                    return Response({"status_code": 200,
                                    "status": "Error",
                                    "message":" Insufficient stock. You don't have enough quantity for the requested product."})
                else:
                    product_ids.append({"productId" :prod.id ,"quantity":i["quantity"] })
            ...
            for product_id in product_ids:
                product = get_object_or_404(Product, id=product_id["productId"])
                price = float(product.sales_price) * product_id["quantity"]
                ...
                product.remaining_quantity-=product_id["quantity"]
                Product.objects.filter(id=product.id).update(remaining_quantity = product.remaining_quantity)
            ...
            invoice_data = Invoice.objects.filter(id=invoice.id).values()
